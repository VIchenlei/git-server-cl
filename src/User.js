...
export default class User {
  constructor (io, userlist, metaStore, socket, cardStore, sqlResultStore, callStore, reportFile, workers, callback, index, alarmStore, crypto, sendCollectorList, handleRssiLogWork) {
    if (socket) {
      ...
      this.meta = new Meta(reportFile, workers, index, metaStore, sendCollectorList)
  ...
  registerEventHandler (socket) {
        ...
        if (req.data.name === 'staff') {
          this.meta.dispatch(this.socket, {
            ...
            data: {
              ...
              name: 'staff',
              op: 'UPDATE',
              sql: `UPDATE dat_staff SET pic = '' WHERE staff_id = ${req.data.keyValue}`
            }
          })
        } else {
          this.meta.dispatch(this.socket, {
            ...
            data: {
              ...
              name: 'vehicle',
              op: 'UPDATE',
              sql: `UPDATE dat_vehicle SET pic = '' WHERE vehicle_id = ${req.data.keyValue}`
            }
          })
        }
      }
      ...
      if (socket.auth) {
        if (req.cmd === 'helpme_done' || req.cmd === 'req_all_data' || req.cmd === 'gas_done') { // 呼救处理消息通过 CALL 从客户端发送过来，通过 PUSH 广播给其他客户端
          ...
          if (req.cmd === 'helpme_done') {
            ...
            Logger.log2db(socket, 4, `${req.data.user_id}解除${req.data.id}的呼救`)
          }
    ...
    socket.on('ALARM', (req) => {
      ...
      if (socket.auth) {
        if (req.cmd === 'alarm_done') { // 告警处理消息通过 ALARM 从客户端发送过来，通过 PUSH 广播给其他客户端
          ...
          Logger.log2db(this.socket, 4, '告警复位成功！')
          // socket.to(config.MONITOR).emit('PUSH', req)
          ...
          Logger.log2db(this.socket, 4, '告警恢复成功！')
        } else {
          Caller.call(socket, req)
        }
      } else {
    ...
    // socket.on('ALL', (msg) => {
    //   this.requestAllCardPositions(msg)
    // })

    // 采集 Server 发往客户端的消息，Web Server 中转（发送到 room : config.MONITOR）
    socket.on('HELP', (req) => {
      // console.log(`Got HELP message : \n\t${JSON.stringify(req)}`)
      ...
      for (let i = 0; i < req.length; i++) {
        ...
        for (var [key, value] of users) {
          ...
          // let sql = `INSERT into his_help_event_user VALUES (1,"lly","lly")`
          // let rows = null
          try {
            mysql.query(sql)
          } catch (err) {
      ...
      //   let resMsg = {
      //     code: -1,
      //     msg: `用户 ${this.name} 尚未登录！`
      //   }
      //   socket.emit('USER', resMsg)
  ...
  async login (req, callback) {
    ...
    let sql = `select user_id, dept_id, role_id, access_id, obj_range, name, is_check, is_limit_by_ip from dat_user where user_id="${userName}" and pwd="${userPass}"`
    let limitIpSql = `select limit_ip_list from dat_user where user_id="${userName}" and pwd="${userPass}" AND is_limit_by_ip=1 AND limit_ip_list LIKE '%${this.socket.request.connection.remoteAddress || this.socket.request.connection.localAddress || this.socket.handshake.address}%'`
    console.log('sql', sql)
    let rows = null
    let limitRows = null
    console.log(limitIpSql)
    try {
      console.log('Going to do login-check on DB， please wait... ', userName)
      limitRows = await mysql.query(limitIpSql)
    ...
    if (rows && rows.length > 0) { // loged in
      if (rows[0].is_limit_by_ip === 0 || (rows[0].is_limit_by_ip === 1 && limitRows.length > 0)) {
        ...
        Logger.log2db(this.socket, 0, '登录系统成功！')
  ...
  logout (req, callback) {
    ...
    if (userInfo) {
      ...
      Logger.log2db(this.socket, 1, '退出系统成功！')
  ...
  async modify (req, callback) {
    ...
    if (rows && rows.length > 0) { // loged in
      ...
      Logger.log2db(this.socket, 3, '修改密码失败！')
    }
