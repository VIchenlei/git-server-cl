...
export default class Meta {
  constructor(reportFile, workers, index, metaStore, sendCollectorList) { // eslint-disable-line
    // let self = this
  ...
  async saveOpDB (socket, req) {
    for (let i = 0, len = req.data.length; i < len; i++) {
      let data = req.data[i]
      Logger.log2db(socket, 4, `手动升井成功！卡号：${data.cardid}`)
    }
  }
  ...
  notifyMeta (req, io) {
    ...
    detail = typeof req.data.id === 'STRING' ? detail.slice(0, 255) : detail // 避免字节长度超出导致添加录入数据库失败
  ...
  checkoutTime (value, row, tableRow, name, socket, tableNames) {
    if (row) {
      row = row[0]
      let newDeleteTime = row && row.lastDelete && new Date(row.lastDelete).getTime() // 数据表中的最后删除时间
      let oldDeleteTime = value && value.lastDelete && new Date(value.lastDelete).getTime() // indexDB中最后删除时间
      let newLastUpdate = row && row.lastUpdate && new Date(row.lastUpdate).getTime() // 数据表中的最后更新时间
      let oldLastUpdate = value && value.lastUpdate && new Date(value.lastUpdate).getTime() // indexDB中的最后更新时间
  ...
  sendDataTable (socket) {
    let promises = []
    let sendTable = ['mdt_update', 'user', 'driver_arrange', 'rt_person_forbid_down_mine', 'credentials_staff']
