...
let options = {
  target: 'http://192.168.118.198:9800', // 目标主机
...
io.on('connection', (socket) => {
  ...
  sessionStore.get(socket.handshake.sessionID, (err, session) => {
    ...
    if (!socket.auth) { // 作为新用户，需登录 // TODO 优化逻辑
      ...
      new User(io, userList, metaStore, socket, cardStore, sqlResultStore, callStore, reportFile, workers, callback, index, alarmStore, crypto, sendCollectorList, handleRssiLogWork)  // eslint-disable-line
    }
  })
})
...
// function updateDatData () {
//   metaStore.updateData()
// }

// function reportFileObjects () {
//   reportFile.checkFile()
// }
...
async function compareMdt (updateTables) {
  ...
  for (let tablename in updateTables) {
    ...
    // console.log(tablename, searchSql)
    try {
...
function onWorkersMessage (msg) {
  ...
  if (cmd === 'METAREQUEST') {
  ...
  } else if (cmd === 'REPTREQUEST' || cmd === 'FEADRUQUIRE') {
    let req = msg.message
    req['key'] = key
    ...
    if (lists.length > 0 && userObj) {
      userObj.meta.sendCollector(lists, userIO)
    }
  }
}
