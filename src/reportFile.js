...
export default class ReportFile {
  ...
  async pullImportMsg (msg) {
    ...
    if (tablename.includes('dat_staff') || tablename.includes('dat_vehicle')) {
      ...
      name = tablename[0] === 'rt_person_forbid_down_mine' ? tablename[0] : tablename[0].slice(4)
    }
  ...
  async broadcastDatas (name, sql) {
    ...
    } else if ((name === 'dept' || name === 'occupation') && typeof sql === 'object') {
  ...
  async updateDB (req) {
    ...
    if (req.data.op === 'DELETE') {
      ...
      if (req.data.name === 'reader_path_tof_n') {
    ...
    if (isImport) {
      console.log(`量导入的id为${req.data.id}`)
    }
