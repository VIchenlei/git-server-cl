...
const metadata = {
  ...
  area: {
    ...
    keyIndex: 0, // table中key值在 field 中的位置
    fields: {
      names: ['area_id', 'name', 'area_type_id', 'business_type', 'map_id', 'over_count_person', 'over_count_vehicle', 'over_time_person', 'over_speed_vehicle', 'path', 'angle', 'is_work_area', 'over_count_person_rp', 'need_display', 'related_area_id', 'related_reader_id', 'related_x', 'related_y', 'related_z'], // 字段
      types: ['NUMBER', 'STRING', 'SELECT', 'NUMBER', 'SELECT', 'NUMBER', 'NUMBER', 'NUMBER', 'STRING', 'STRING', 'NUMBER', 'SELECT', 'NUMBER', 'SELECT', 'SELECT', 'SELECT', 'NUMBER', 'NUMBER', 'NUMBER'], // 字段类型
      labels: ['区域编号', '区域名称', '区域类型', '区域业务', '所属地图', '人数上限', '车辆上限', '人停留时长上限(分钟)', '车速上限(Km/h)', '区域定义', '车辆角度', '是否是工作区域', '区域核对人数上限', '是否对外展示', '关联区域', '关联分站', '关联坐标x', '关联坐标y', '关联坐标z'],
      enableNull: [false, false, false, false, false, false, false, false, false, false, true, true, true, false, true, true, true, true, true]
    }
  },
  area_business: {
    name: 'area_business',
  ...
  coalface: {
    ...
    fields: {
      ...
      types: ['SELECT', 'NUMBER', 'DATE', 'NUMBER', 'NUMBER', 'NUMBER', 'DATE', 'STRING', 'NUMBER', 'NUMBER', 'NUMBER', 'NUMBER', 'NUMBER', 'NUMBER', 'NUMBER', 'SELECT', 'NUMBER'], // 字段类型
  ...
  */
  ...
  staff: {
    ...
    fields: {
      ...
      enableNull: [false, false, true, false, false, true, true, true, true, true, true, true, true, true, true, true, true]
    }
  },
...
*/
  ...
  vehicle_extend: {
    ...
    fields: {
      ...
      types: ['NUMBER', 'SELECT', 'SELECT', 'NUMBER', 'SELECT', 'NUMBER', 'NUMBER'],
      labels: ['车辆编号', '部门', '班组', '卡号', '班制', '是否显示(0:不显示;1:显示)', '是否推送电量低(0:不推送;1:推送)'],
  ...
  */
  ...
  reader: {
    ...
    keyIndex: 0, // table中key值在 field 中的位置
    fields: {
      names: ['reader_id', 'reader_type_id', 'name', 'brief_name', 'map_id', 'area_id', 'x', 'y', 'z', 'angle', 'state', 'ip', 'device_type_id', 'dimension', 'enable_simulate_card', 'need_power_alarm', 'fade_value'], // 字段
      types: ['NUMBER', 'SELECT', 'STRING', 'STRING', 'SELECT', 'SELECT', 'NUMBER', 'NUMBER', 'NUMBER', 'NUMBER', 'SELECT', 'STRING', 'SELECT', 'SELECT', 'SELECT', 'SELECT', 'NUMBER'], // 字段类型
      labels: ['分站编号', '分站类型', '分站名称', '分站简称', '所属地图', '所属区域', '坐标X', '坐标Y', '坐标Z', '分站仰角', '分站状态', 'IP', '设备类型', '定位维度', '是否可以模拟成定位卡', '掉电告警', '盲区阈值'],
      enableNull: [false, false, false, true, false, false, false, false, true, false, false, true, false, false, false, false, true]
  ...
  user: { // 密码只能在server端读写，不会push到客户端
    ...
    keyIndex: 0, // table中key值在 field 中的位置
    fields: {
      names: ['user_id', 'pwd', 'name', 'role_id', 'is_check', 'access_id', 'is_limit_by_ip', 'limit_ip_list'], // 字段
      types: ['STRING', 'STRING', 'STRING', 'SELECT', 'SELECT', 'STRING', 'SELECT', 'STRING'], // 字段类型
      labels: ['账号', '密码', '用户名称', '用户角色', '用户类别', '管理范围', '是否启用限制IP登录', '限制登录IP'],
      enableNull: [false, false, false, false, false, false, true, false, true]
    }
  },
  role: {
    name: 'role',
  ...
  */
  ...
  att_rule: {
    ...
    keyIndex: 0, // table中key值在 field 中的位置
    fields: {
      names: ['att_rule_id', 'name', 'offset_pre', 'offset_back', 'description'], // 字段, md5用于更新地图
      types: ['NUMBER', 'STRING', 'NUMBER', 'NUMBER', 'STRING'], // 字段类型
      labels: ['考勤规则id', '名称', '考勤向前偏移时间', '考勤向前偏移时间', '描述'],
      enableNull: [false, false, false, false, true]
    }
  },
  ...
  att_rule_vehicle_type: {
    ...
    keyIndex: 0, // table中key值在 field 中的位置
    fields: {
      names: ['vehicle_type_id', 'att_rule_id'], // 字段, md5用于更新地图
      types: ['NUMBER', 'NUMBER'], // 字段类型
      labels: ['车辆类型ID', '考勤规则ID'],
      enableNull: [false, false]
    }
  },
  att_rule_area: {
    ...
    keyIndex: 0, // table中key值在 field 中的位置
    fields: {
      names: ['area_id', 'att_rule_id'], // 字段, md5用于更新地图
      types: ['NUMBER', 'NUMBER'], // 字段类型
      labels: ['区域ID', '考勤规则ID'],
      enableNull: [false, false]
    }
  ...
  //   keyIndex: 0, // table中key值在 field 中的位置
  //   fields: {
  //     names: ['id', 'staff_id', 'start_time', 'end_time', 'status', 'oper_time', 'oper_user'], // 字段, md5用于更新地图
  //     types: ['NUMBER', 'NUMBER', 'DATETIME', 'DATETIME', 'SELECT', 'DATETIME', 'STRING'], // 字段类型
  //     labels: ['记录保存数', '员工号', '禁止下井起始时间', '禁止下井结束时间', '本记录是否启用', '新增操作时间', '新增操作员'],
  //     enableNull: [false, false, false, false, false, false, false]
  //   }
  // },
  ...
  goaf: {
    ...
    keyIndex: 0,
    fields: {
      names: ['goaf_id', 'name', 'path', 'need_display'],
      types: ['NUMBER', 'STRING', 'STRING', 'SELECT'],
      labels: ['编号', '名称', '路径', '检查人员是否可见'],
      enableNull: [false, false, false, false]
    }
  },
  dev_pos_module: {
